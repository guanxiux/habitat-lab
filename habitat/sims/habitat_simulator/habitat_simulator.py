...
class AgentConfig:
    def __init__(self, agent_config: Config, sensor_suite: SensorSuite):
...
class HabitatSim(habitat_sim.Simulator, Simulator):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.num_agent = len(self.habitat_config.AGENTS)
        _agent_config = []
        for i in range(self.num_agent):
            agent_config = self._get_agent_config(i)
            ...
            _agent_config.append(AgentConfig(agent_config, SensorSuite(sim_sensors)))

        self._agent_config = _agent_config
        self._sensor_suite = _agent_config[self.habitat_config.DEFAULT_AGENT_ID].sensor_suite
        self.sim_config = self._create_sim_config(_agent_config)
    ...
    def _create_sim_config(
        self, _agent_config: List
    ) -> habitat_sim.Configuration:
        sim_config = habitat_sim.SimulatorConfiguration()
        # Check if Habitat-Sim is post Scene Config Update
        if not hasattr(sim_config, "scene_id"):
        ...
        sim_config.scene_id = self.habitat_config.SCENE
        agent_configs = []
        for c in _agent_config:
            old_agent_config = c.agent_config
            agent_config = habitat_sim.AgentConfiguration()
            overwrite_config(
                config_from=old_agent_config,
                config_to=agent_config,
                # These keys are only used by Hab-Lab
                ignore_keys={
                    "is_set_start_state",
    ...
    def step(self, action: Union[str, np.ndarray, int]) -> Observations:
        ...
        if isinstance(action, MutableMapping):
            if self._default_agent_id not in sim_obs:
                sim_obs = self.get_sensor_observations()
            else:
                sim_obs = sim_obs[self._default_agent_id]
        self._prev_sim_obs = sim_obs
        observations = self._sensor_suite.get_observations(sim_obs)
        return observations

    def get_observations_of(self, agent_id: Union[int, List]) -> Observations:
        ...
        if isinstance(agent_id, int):
            ...
            obs = {}
            for i in agent_id:
                obs[i] = self._agent_config[i].sensor_suite.get_observations(sim_obs[i])
            return obs
        else:
    ...
    def reconfigure(
        ...
        self.sim_config = self._create_sim_config(self._agent_config)
